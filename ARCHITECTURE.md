...
Kedro-viz currently utilizes two different methods of data ingestion: the Redux setup for ingesting data for the pipeline and flowchart-view related components, and Graphql via Apollo Client for ingesting data for the experiment tracking components. 
...
All experiment tracking data are stored in a SQLite database that is generated automatically once [experiment tracking is enabled in your Kedro project](https://kedro.readthedocs.io/en/stable/08_logging/02_experiment_tracking.html). On loading Kedro-viz from the Kedro project, the Kedro-viz backend will consume the run data stored in the database and serve the data via the graphql endpoint via graphql query requests from the Apollo client on the Front end. 
...
The following sections outlines the Redux state management used to manage the data and app state for the flowchart view. The data for the runs view ( experiment tracking features ) are managed by React hooks and the Apollo GraphQL client. 
...
The `src/apollo` directory contains all the related setup for ingesting data from the graphQL endpoint for experiment tracking features, including the schema that defines all the queries, mutation and subscription types, the config that sets up the Apollo Client to be used within react components, and other files wuth helper functions such as mocks to generate random data for the mock server. 

Please note that the schema checked in this folder is the contract that unites both the BE and FE expectations of the graphQL endpoint, and should always be up to date with the schema of the graphQL endpoint. 
## Utils
