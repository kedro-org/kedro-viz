...
def _service(
    ...
    *,
    ...
    return NodeMetadataService.from_inspection_inputs(
        InspectionInputs(
            snapshot=snapshot,
            catalog_config={},
            parameters=parameters or {},
        ),
        enrichment=node_extras_by_name,
        live_nodes_by_id=live_nodes_by_id,
    )


def test_task_enrichment_uses_live_parameters_and_preserves_static_fields() -> None:
    ...
    live_task.parameters = {"options": {"value": 99}}
    service = _service(
        _snapshot([snapshot_node]),
        parameters={"options": {"value": 3}},
    ...
    assert response.parameters == {"options": {"value": 99}}
...
def test_data_enrichment_releases_once_per_lookup_and_respects_preview_policy() -> None:
    ...
    first_preview = service.get_node_metadata_response(data_id)
    ...
    assert first_preview == with_preview
    assert first_preview is not with_preview
    assert with_preview.preview == "live preview"
    assert with_preview.preview_type == "str"
    assert dataset.release_calls == 3
    assert dataset.preview_calls == 2

    static_response = service._builder.build(data_id)
    assert isinstance(static_response, DataNodeMetadataAPIResponse)
    assert static_response.type == "test.RawDataset"
    assert static_response.filepath == "static.csv"
    assert static_response.preview is None
    assert static_response.preview_type is None
    assert dataset.release_calls == 3
    assert dataset.preview_calls == 2


@pytest.mark.parametrize(
    "dataset_class,extension", [(CSVDataset, "csv"), (ParquetDataset, "pq")]
)
def test_missing_preview_file_preserves_metadata(
    dataset_class, extension, tmp_path, caplog
) -> None:
    ...
    live_data = GraphNode.create_data_node(
        ...
        layer=None,
        tags=set(),
        dataset=dataset_class(filepath=str(filepath)),
        node_extras=None,
        modular_pipelines=set(),
    )
    service = _service(
        _snapshot([_node("produce", outputs=["data"])]),
        node_extras_by_name={"data": NodeExtras(stats={"rows": 3})},
        live_nodes_by_id={data_id: live_data},
    )
    ...
    assert "preview" not in response.model_dump(mode="json", exclude_none=True)
    assert response.filepath == str(filepath)
    assert response.stats == {"rows": 3}
    assert response.run_command == "kedro run --to-outputs=data"
    assert "'data' could not be previewed" in caplog.text
...
def test_graph_requests_do_not_read_or_release_live_metadata() -> None:
    data_id = _create_dataset_node_id("data")
    dataset = _PreviewDataset()
    live_data = GraphNode.create_data_node(
        dataset_id=data_id,
    ...
    static_service = _service(snapshot)

    assert context.graph.get_pipeline_response()
    assert not hasattr(context.graph, "_live_nodes_by_id")
    assert dataset.release_calls == 0
    assert dataset.preview_calls == 0

    enriched = context.node_metadata.get_node_metadata_response(data_id)
    static = static_service.get_node_metadata_response(data_id)

    assert isinstance(enriched, DataNodeMetadataAPIResponse)
    assert isinstance(static, DataNodeMetadataAPIResponse)
    assert enriched.preview == "live preview"
    assert static.preview is None
    assert static.filepath is None
    assert dataset.release_calls == 1
...
def test_parameter_nodes_use_values_from_the_live_catalog() -> None:
    ...
    assert response.parameters == {"value": 99}
    assert service._live_nodes_by_id == {parameter_id: live_parameter}


def test_all_parameters_node_uses_values_from_the_live_catalog() -> None:
    snapshot_node = _node("consume", inputs=["parameters"])
    parameter_id = _create_dataset_node_id("parameters")
    live_parameter = GraphNode.create_parameters_node(
        dataset_id=parameter_id,
        dataset_name="parameters",
        layer=None,
        tags=set(),
        parameters=MemoryDataset(data={"value": 99}),
        modular_pipelines=set(),
    )
    assert isinstance(live_parameter, ParametersNode)
    service = _service(
        _snapshot([snapshot_node], inputs=["parameters"]),
        parameters={"value": 1},
        live_nodes_by_id={parameter_id: live_parameter},
    )
...
def test_invalid_live_all_parameters_value_preserves_static_metadata() -> None:
    snapshot_node = _node("consume", inputs=["parameters"])
    parameter_id = _create_dataset_node_id("parameters")
    live_parameter = GraphNode.create_parameters_node(
        dataset_id=parameter_id,
