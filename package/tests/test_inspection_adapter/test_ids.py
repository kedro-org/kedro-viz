"""Tests for the inspection adapter ID scheme (the new Viz-side node-ID scheme).
...
def test_task_node_id_excludes_tags() -> None:
    """Re-tagging a node must not change its ID.

    The ID is the hash of exactly ``(name, inputs, outputs)`` — tags (and any other field) are not
    part of the signature, so they cannot feed in. Pinning the field set here guards against a
    future change that accidentally folds an extra field into the hash.
    """
