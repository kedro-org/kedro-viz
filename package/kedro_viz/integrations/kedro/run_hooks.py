...
from kedro_viz.integrations.kedro.hooks_utils import hash_node, create_dataset_event, write_events, generate_timestamp
...
class PipelineRunHooks:
    ...
    def before_pipeline_run(self, run_params: dict, pipeline, catalog) -> None:
        ...
        self._add_event({"event": "before_pipeline_run", "timestamp": generate_timestamp()}, True)
    ...
    def after_dataset_loaded(self, dataset_name: str, data: Any, node: KedroNode = None) -> None:
        self._add_event(create_dataset_event("after_dataset_loaded", dataset_name, data, self.datasets))
        self._clear_context()
    ...
    def after_dataset_saved(self, dataset_name: str, data: Any, node: KedroNode = None) -> None:
        self._add_event(create_dataset_event("after_dataset_saved", dataset_name, data, self.datasets))
        self._clear_context()
    ...
    def after_pipeline_run(self, run_params) -> None:
        if run_params.get("pipeline_name"):
            return
        self._add_event({"event": "after_pipeline_run", "timestamp": generate_timestamp()}, True)
    ...
    def on_pipeline_error(self, error: Exception) -> None:
        ...
        event = {"event": "on_pipeline_error", "error": str(error), "timestamp": generate_timestamp()}
        
        if self._current_dataset:
            event.update({"dataset": self._current_dataset, "operation": self._current_operation })
        if self._current_node:
            event.update({"node": self._current_node.name, "node_id": hash_node(self._current_node)})
        else:
            # find first unstarted node for hint
            for n in self._all_nodes:
                if n.name not in self._started_nodes:
                    event.update({"node": n.name, "node_id": hash_node(n), "status": "not_started"})
                    break
