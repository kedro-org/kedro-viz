...
def _get_import_statements_from_ast(parsed_content_ast_node: ast.Module) -> List[str]:
...
def _get_unresolvable_imports(
    file_path: Path, import_statements: List[str]
) -> List[str]:
    unresolvable_imports: List[str] = []

    for statement in import_statements:
        if _is_valid_import_stmt(statement):
            if statement.startswith("import "):
                ...
                if _is_relative_import_resolvable(file_path, module_name):
                    continue
...
def _parse_project_for_imports(project_path: Path) -> Dict[Path, List[str]]:
...
def get_mocked_modules(project_path: Path) -> Dict[str, MagicMock]:
    all_imports: Dict[Path, List[str]] = _parse_project_for_imports(project_path)
    mock_modules: Dict[str, MagicMock] = {}

    for file_path, imports in all_imports.items():
        unresolvable_imports: List[str] = _get_unresolvable_imports(file_path, imports)
