...
class TaskNodeMetadata(GraphNodeMetadata):
    ...
    def set_preview(cls, _):
        try:
            ...
            # Serialize PreviewPayload to dictionary
            serialized_preview = {
                "kind": preview_payload.kind,
                "content": preview_payload.content
            }
            
            # Include meta only if it exists and is not None
            if preview_payload.meta is not None:
                serialized_preview["meta"] = preview_payload.meta
