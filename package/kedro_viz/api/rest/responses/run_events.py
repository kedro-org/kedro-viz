...
def _extract_pipeline_timing_and_status(
    ...
    end_events = [
        ...
        if event.get("event") in {EventType.AFTER_PIPELINE_RUN, EventType.ON_PIPELINE_ERROR}
    ]
...
def _finalize_pipeline_info(
    ...
    node_durations = {
        node_id: node.duration_sec for node_id, node in nodes.items()
    }
...
def transform_events_to_structured_format(
    ...
    for event in events:
        ...
        elif event_type in {EventType.AFTER_DATASET_LOADED, EventType.AFTER_DATASET_SAVED}:
...
def get_run_events_response() -> StructuredRunEventAPIResponse:
    ...
    try:
        ...
        if not kedro_project_path:
            logger.warning("Could not find a Kedro project to load pipeline events file")
            return StructuredRunEventAPIResponse()
